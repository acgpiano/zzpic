...
var search = {
    ...
    createNewTab : function(url){
      chrome.tabs.create({url:url + '&uptype=upload_pc'});
    },
...
// 更新检测
var Updater = function(ajax, version, updateUrl){
  this.ajax = ajax || window.ajax || {};
  this.version = version || null;
  this.url = updateUrl || null;
  this.init();
}
Updater.prototype = {
  isNewVersion: function(v){
    var oldVersion = this.version.split('.');
    var newVersion = v.split('.');
    for(var i = 0, len = oldVersion.length; i < len; i++){
      if(newVersion[i] && oldVersion[i] && oldVersion[i] < newVersion[i]){
        return true;
      }
    }
    return false;
  ...
  update : function(){
    ...
    if(me.version){
      me.ajax({
        url : 'http://img0.bdstatic.com/img/image/wantu/expansion/version.txt',
        method : "get",
        success : function(result){
          var newstVersion = JSON.parse(result)['chrome'] || 0;
          if(me.isNewVersion(newstVersion)){
            var note = Note.show({
              title: Local.getLocalText('update_title'),
              icon: "images/icon_48.png",
              body: Local.getLocalText("update_body")
            });
            chrome.extension.sendMessage({
              msg : 'log_messages',
              param : {
            ...
            if(me.url){
              note.onclick = function(){
                chrome.tabs.create({'url': me.url});
                HotKey.set('updater', newstVersion);
                chrome.extension.sendMessage({
                  msg : 'log_messages',
                  param : {
                    type : 'fe',
                    fe : {
                      logid : '0200000201',// 桌面提醒
                      ac : 'download'
                    }
                  }
                });
  ...
  init : function(obj){
    obj = obj || {};
    this.ajax = obj.ajax || window.ajax || {};
    this.update();
  }
}
...
var app = {
  version : HotKey.get('updater') || chrome.app.getDetails().version,
  updateUrl : 'http://image.baidu.com/detail/expansion',
  init : function(){
    ...
    var u = new Updater(window.ajax, app.version, app.updateUrl);
