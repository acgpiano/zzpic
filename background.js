var screenshot = {
	...
	postImage: function(data, url) {
		chrome.tabs.create({
			'url': url || data || screenshot.canvas.toDataURL("image/png")
		});
	},
...
var Note = {
	...
	show: function(o, timeout) {
		var note = new Notification(o.title, {
			icon: o.icon,
			body: o.body
		});
		if (o.replaceId) {
...
var search = {
	...
	searchBySrc: function(imageInfo) {
		...
		if (imgOption == "baidu") {
			...
			search.createNewTab(url);
		} else if(imgOption == "360"){
			search.engine360(src);
		}else{
			search.createNewTab(localStorage.selfurl + src);
		}
	},
	...
	success: function(response) {
		...
		setTimeout(function() { //remove the progress bar shadow
			...
			if (imgOpt == "baidu") {
				...
				search.createNewTab(localStorage.selfurl + response.substr(80));
			}
		}, 10);
	},
